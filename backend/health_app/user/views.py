...
from health_backend_app.custom_permission import IsVerified,IsRegistered,IsTncVerified,IsSubscribed
...
from health.models import Contract,ContractRecipient
from .custom_pagination import BuyerContractPagenation
...
class BuyerSpentView(APIView):
    permission_classes=[IsAuthenticated,IsVerified,IsRegistered]
    authentication_classes = []
    def get(self, request, *args, **kwargs):
        buyer = request.user
        if buyer.is_buyer == False:
            return wrap_response(
                success=False,
                code='permission_denied',
                errors=[{"field":"user","message": "You don't have permission to access this resource."}],
                status_code=status.HTTP_403_FORBIDDEN
            )
        contracts_obj = ContractRecipient.objects.filter(buyer=buyer,is_purchased=True).select_related('contract').aggregate(Sum('price') , Count('contract_recipient_id'))
        spent_value = round(contracts_obj['price__sum'],2) if contracts_obj['price__sum'] else 0
        contract_count = contracts_obj['contract_recipient_id__count'] if contracts_obj['contract_recipient_id__count'] else 0
        active_user_count = User.objects.filter(is_registered=True).count()
        return wrap_response(
            success=True,
            code='buyer_spent_retrieved',
            data={"buyer_spent":spent_value , "no_of_contracts":contract_count , 'active_user_count':active_user_count},
            status_code=status.HTTP_200_OK
        )
...
class SellerRevenueView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        contracts = Contract.objects.filter(seller=seller)
        active_user_count = User.objects.filter(is_registered=True).count()
        if not contracts:
            return wrap_response(
            success=True,
            code='seller_revenue_retrieved',
            data={"seller_revenue":0 , "no_of_contracts":0 , 'active_user_count':active_user_count},
            status_code=status.HTTP_200_OK
            )

        purchased_contracts = ContractRecipient.objects.filter(
            contract__in=contracts,
            is_purchased=True
        )
        aggregated_data = purchased_contracts.aggregate(
            total_revenue=Sum('price'),
            contract_count=Count('contract_id', distinct=True)  
        )
        total_revenue = round(aggregated_data['total_revenue'],2) or 0
        contract_count = aggregated_data['contract_count'] or 0
        
        return wrap_response(
            success=True,
            code='seller_revenue_retrieved',
            data={"seller_revenue":total_revenue , "no_of_contracts":contract_count , 'active_user_count':active_user_count},
            status_code=status.HTTP_200_OK
        )
...
class BuyerContractView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        buyer = request.user
        queryset = ContractRecipient.objects.filter(buyer=buyer , is_purchased=True).values('created_at','contract_recipient_id')

        paginator = BuyerContractPagenation()
